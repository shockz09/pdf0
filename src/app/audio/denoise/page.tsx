...
export default function DenoiseAudioPage() {
	...
	const processFile = useCallback(
		async (fileToProcess: File, denoiseStrength: DenoiseStrength) => {
			...
			try {
				...
				const blob = await denoiseAudio(fileToProcess, denoiseStrength, (p) =>
					setProgress(p),
				);
				...
				setError(
					err instanceof Error ? err.message : "Failed to denoise audio",
				);
			} finally {
						...
						<div className="grid grid-cols-3 gap-2">
							{strengthOptions.map((opt) => (
								...
								>
									...
									<span
										className={`block text-xs ${strength === opt.value ? "text-background/70" : "text-muted-foreground"}`}
									>
										{opt.desc}
									</span>
								</button>
